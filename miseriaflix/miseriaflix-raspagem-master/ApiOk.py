...
cur.execute("select termo, filtro, duration from pesquisa1")
...
for termos_row in termos_rows:
    ...
    duration = termos_row[2].strip()  # Obter a duração e remover espaços em branco

    for search_text in termos:
        url = "https://ok.ru/web-api/v2/search/portal"

        duration = duration if duration in ['LONG', 'SHORT'] else '' 
